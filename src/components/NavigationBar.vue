        ...
        >
            ...
            <div class="isIndex box" v-if="isIndex">
                <div class="h_search">
                    ...
                    <div @click.stop="activeBtnClick" class="top_sort fl clearfix">
                        <span class="selected fl color-333 fontbold">{{ activeTitle }}</span>
                        <!-- 箭头方向加active -->
                        <span class="icon icon_arrow fl" :class="active ? 'active' : ''"></span>
                    </div>
                    <div v-if="active" class="select_box color-333 f36">
                        <div
                            @click="squareBtnClick(item)"
                            class="option"
            ...
            <div class="box" v-if="!isIndex && !isMine">
                <div class="">
                    ...
                    >
                        <image v-if="isFromShare" src="/static/tabs/home.png" style="width: 44rpx;height: 44rpx;transform:translateY(-22rpx)" alt="" />
                        <image v-else src="/static/tabs/back.svg" style="" alt="" />
                    </div>
                    <div
                        class="title"
                        v-if="title"
...
export default {
    ...
    onShow() {
        ...
        let prevPageRoute = pages[0].__displayReporter.showReferpagepath
        if(prevPageRoute === 'pages/login/main.html') {
            this.activeTitle = "广场"
        }
    },
    created() {
    ...
    methods: {
        ...
        activeBtnClick() {
            this.$emit('activeBtnClick')
            // this.active = !this.active
        },
        squareBtnClick(item) {
            this.active = false
            this.activeTitle = item.word
            this.$emit('activeTitleClick', item)
        }
    },
    onHide() {
        this.active = false
